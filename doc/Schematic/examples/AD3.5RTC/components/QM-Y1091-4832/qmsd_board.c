...
void qmsd_board_init_screen() {
    ...
    gpio_set_pull_mode(LCD_D0_PIN, GPIO_PULLUP_ONLY);
    gpio_set_direction(LCD_D0_PIN, GPIO_MODE_INPUT);
    gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[LCD_D3_PIN], PIN_FUNC_GPIO);
    gpio_set_pull_mode(LCD_D3_PIN, GPIO_PULLUP_ONLY);
    gpio_set_direction(LCD_D3_PIN, GPIO_MODE_INPUT);
    board_screen_select = gpio_get_level(LCD_D0_PIN) << 4 | gpio_get_level(LCD_D3_PIN);
